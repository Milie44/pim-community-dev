 ...
 */
class RegisterJobInstanceFormTypeSubscriberPass implements CompilerPassInterface
{
    /** @staticvar int The default provider priority */
    const DEFAULT_PRIORITY = 100;

    /** @staticvar string The registry id */
    const REGISTRY_ID = 'pim_import_export.form.type.job_instance';

    /** @staticvar string */
    const SERVICE_TAG = 'form.job_instance';
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($providers as $unsortedProviders) {
            foreach ($unsortedProviders as $provider) {
                $registryDefinition->addMethodCall('addEventSubscriber', [$provider]);
            }
        }
    }
}
